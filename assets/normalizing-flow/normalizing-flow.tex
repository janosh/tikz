...
\begin{tikzpicture}[
  ...
  \draw[flow] (z1) -- node[fill=white, pos=0.15, inner sep=1] {\textbf\dots} node[above, midway] {$f_i(z_{i-1})$} (zi);

  \node[znode, draw=green!70!black, right=2.5 of zip1] (zk) {$z_k$};
  \draw[flow] (zip1) -- node[fill=white, pos=0.15, inner sep=1] {\textbf\dots} node[above, midway] {$f_k(z_{k-1})$} (zk);
