...
\begin{tikzpicture}[
    ...
    stage/.style={shape=circle, draw, font=\bfseries, minimum width=\circSize},
  ...
  \begin{scope}[local bounding box=challenges]
    ...
    \foreach \itm [count=\i, evaluate={\a=\i*15+120;}] in
      {volume, velocity, variety, veracity, visualization, long-term storage, standardization} {
        \node[challenge] at (\a:\circSize/2 + 2mm) [rotate=\a+180, anchor=east] {\itm};
        \draw (\a:\circSize/2 + 2mm) -- (\a:\circSize/2);
      }
    ...
    \begin{scope}[xshift=6cm]
      ...
      \foreach \itm [count=\i, evaluate={\a=30-\i*15;}] in
        {efficiency, speed, compactness} {
          \node[challenge] at (\a:\circSize/2 + 2mm) [rotate=\a, anchor=west] {\itm};
          \draw (\a:\circSize/2 + 2mm) -- (\a:\circSize/2);
        }
    \end{scope}
